create or replace package fs11_generation as
    procedure generate_incoming_file_and_env;
end fs11_generation;
create or replace package body fs11_generation as
    type mcc_rules_table is table of FS11_MCC_RULES%rowtype;
    mcc_rules mcc_rules_table;
    type merchant_rules_table is table of FS11_MERCHANT_RULES%rowtype;
    merchant_rules merchant_rules_table;
    ...
    begin
        ...
        for tc in (select distinct table_name
                   from user_constraints
                   where table_name like 'FS11_%')
            loop
                execute immediate 'truncate table ' || tc.table_name;
                print('Truncated: ' || tc.table_name);
            end loop;
    ...
    procedure generate_incoming_file_and_env as
    begin
        init_collections;
        trunc_tables;
