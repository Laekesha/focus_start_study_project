...
| 2          | an..12        | Уникальный идентификатор файла в системе отправителя. Не допустима обработка файлов с одинаковыми идентификаторами |
...
| 3          | an..12           | Уникальный идентификатор транзакции покупки в системе учёта мерчанта. Не допустима обработка транзакции с одинаковыми идентификаторами от одного мерчанта |
...
| 2          | an40             | SHA1-криптограмма номера карты. Может отличаться от карты, по которой была совершена покупка. Должна принадлежать тому же клиенту, что и карта покупки     |
| 3          | an..12           | Уникальный идентификатор транзакции возврата в системе учёта мерчанта. Не допустима обработка транзакции с одинаковыми идентификаторами от одного мерчанта |
| 4          | n14              | Дата и время совершения возврата в формате YYYYMMDDHH24MISS                                                                                                |
| 5          | n..10            | Сумма возврата в минимальных единицах валюты RUB. По всем обработанным возвратам совокупно не допустимо превышение над суммой оригинальной покупки         |
| 6          | an..30           | Идентификатор мерчанта                                                                                                                                     |
| 7          | an..12           | Уникальный идентификатор транзакции покупки, для которой выполняется возврат. Возможно наличие нескольких возвратов для одной покупки                      |
| 8          | ans..2000 (опц.) | Дополнительное текстовое описание                                                                                                                          |
...
| 5          | n..10         | Размер кэшбэка к выплате на текущий момент в минимальных единицах валюты RUB |
